...
Then, the files should be added to the `data` folder. We could not keep these files on GitHub due to space limitations. From these files, only the `challenge_set.csv` and `final_submission_set.csv` are actually needed, together with the `.parquet` files provided by the challenge. The other ones are intermediary files generated by our code, but we are providing them in case someone cannot run all the steps needed to build the models.
